...
:mainpage: link:https://github.com/tobilinz/homepage-foschingsball[Hauptseite dieses Repositories]
...
Alle Daten, die zur Webseite gehören, werden auf einem Server in verschieden Ordnern gespeichert. Wenn jemand die Webseite öffnet, dann erhält die Person diese Dateien und sie werden in Form einer Webseite angezeigt. +
 +
Ein GitHub Repository wie dieses hilft dabei, die Webseite zu programmieren, indem Änderungen an Code zwischen Geräten einfacher synchronisiert werden können. Damit fällt es einem Entwicklungsteam einfacher, gemeinsam an der Webseite zu arbeiten. Ein GitHub Repository ist allerdings nicht notwendig.
...
Ob die Header richtig konfiguriert sind, kann mithilfe des Tools <<Security Headers>> überprüft werden.
...
Um die Webentwicklung zu erleichern, wird die Erweiterung link:https://marketplace.visualstudio.com/items?itemName=ritwickdey.LiveServer[Live Server] empfohlen.
...
Hierfür muss zuerst wieder das WebFTP Portal, welches auch zum herunterladen der 'alten' Dateien verwendet wurde, geöffnet werden.
...
link:https://pagespeed.web.dev[PageSpeed Insights] testet, ob Webseiten gut für Google und andere Suchmaschienen optimiert sind. Das Werkzeug kann auch dabei helfen, die Webseitenerfahrung für Nutzer zu verbessern.
...
link:https://securityheaders.com[Security Headers] testet, wie der Name schon sagt, ob die Header-Informationen, die an Nutzer gesendet werden, wenn sie die Webseite aufrufen, auf Sicherheitslücken. Die Header können in den `.htaccess`-Dateien, im Menü <<Serverkonfiguration>>, bearbeitet werden.
